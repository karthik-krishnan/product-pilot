export function fixINVESTTemplate({
  principleKey,
  principleLabel,
  title,
  asA,
  ...
  return `A user story is failing the "${principleLabel}" INVEST principle. Propose the minimal concrete fix that resolves the issue.
...
FAILING PRINCIPLE: ${principleLabel} (score: ${score}/100)
FEEDBACK: ${feedback}
SUGGESTIONS:
${suggestions}
...
Use this when the Small principle fails and the best fix is decomposing the story.
{
  ...
  "splitNewStory": {
    ...
    "priority": "${priority}", "storyPoints": <number>
  },
